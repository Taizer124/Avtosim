...
namespace Assets.VehicleController
{
    ...
    public class CarVisualsExtra : MonoBehaviour
    {
        ...
        private void DestroyAllEffects()
        {
            ...
            // Дым и следы инстанцируют собственные объекты и цепляют их к
            // колёсам. Раньше здесь ссылка просто обнулялась — объекты
            // оставались висеть, и если в момент выключения машины дым играл,
            // остановить его было уже некому (он дымил вечно), а на каждом
            // следующем включении создавался ещё один комплект.
            if (_tireSmoke != null)
                _tireSmoke.Destroy();
            if (_tireTrails != null)
                _tireTrails.Destroy();
