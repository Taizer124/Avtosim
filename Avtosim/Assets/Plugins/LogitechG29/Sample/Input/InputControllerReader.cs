...
namespace LogitechG29.Sample.Input
{
        ...
        InputController.ITransmissionActions
    {
        ...
        // Инициализирован ли ввод. Потребители (например AllInOneInputProvider)
        // проверяют это и вызывают EnsureInitialized() сами: OnEnable у
        // ScriptableObject-ассета привязан к моменту ЗАГРУЗКИ ассета, а не к
        // входу в Play Mode, поэтому полагаться на него нельзя — карты ввода
        // могли ни разу не включиться, и тогда мёртв весь ввод (клавиатура,
        // Logitech, MOZA).
        public bool IsInitialized => _input != null;

        /// <summary>
        /// Создаёт InputController и включает все карты ввода. Идемпотентен —
        /// безопасно вызывать из нескольких провайдеров и каждый кадр.
        /// </summary>
        public void EnsureInitialized()
        {
            if (!Application.isPlaying || _input != null)
                return;
            ...
            Debug.Log("[InputControllerReader] Input actions initialized and enabled");
        }
